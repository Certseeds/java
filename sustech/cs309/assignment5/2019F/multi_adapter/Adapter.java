...
public class Adapter implements FileOperateInterfaceV2Impl, ManageStaffInterface {
    private final FileOperateInterfaceV1 adaptee_FileOperateInterface;
    private final ManageStaffInterface adaptee_ManageStaffInterface;
    ...
    public void writeByRoom(List<StaffModel> list) {
        var temp = list.toArray(new StaffModel[0]);
        System.out.println(Arrays.toString(temp));
        Arrays.sort(temp, Comparator.comparing(StaffModel::getRoom));
        System.out.println(Arrays.toString(temp));
        adaptee_FileOperateInterface.writeStaffFile(Arrays.stream(temp).collect(Collectors.toList()));
    }
